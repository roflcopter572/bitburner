...
const hackScripts = ['/suite/hack.js', '/suite/grow.js', '/suite/weaken.js']
  ...
  while (true) {
    ...
    if (action === 'weaken') {
      ...
      for (let i = 0; i < hackableServers.length; i++) {
        ...
        if (growCycles) {
          await ns.exec('/suite/grow.js', server.host, cyclesToRun, bestTarget, cyclesToRun, growDelay, createUUID())
          growCycles -= cyclesToRun
          cyclesFittable -= cyclesToRun
        }

        if (cyclesFittable) {
          await ns.exec('/suite/weaken.js', server.host, cyclesFittable, bestTarget, cyclesFittable, 0, createUUID())
      ...
      for (let i = 0; i < hackableServers.length; i++) {
        ...
        if (hackCycles) {
          await ns.exec('/suite/hack.js', server.host, cyclesToRun, bestTarget, cyclesToRun, hackDelay, createUUID())
        ...
        if (cyclesFittable && growCycles) {
          ...
          await ns.exec('/suite/grow.js', server.host, growCyclesToRun, bestTarget, growCyclesToRun, growDelay, createUUID())
